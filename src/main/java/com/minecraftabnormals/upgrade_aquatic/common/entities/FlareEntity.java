...
public class FlareEntity extends FlyingEntity {
	...
	class OrbitPointGoal extends FlareEntity.MoveGoal {
	    ...
	    private void selectNext() {
	        ...
	        FlareEntity.this.orbitOffset = (new Vector3d(FlareEntity.this.orbitPosition.getX(), FlareEntity.this.orbitPosition.getY(), FlareEntity.this.orbitPosition.getZ())).add((double)(this.distance * MathHelper.cos(this.angle)), (double)(-4.0F + this.height), (double)(this.distance * MathHelper.sin(this.angle)));
	    }
	}
	...
	class PickAttackGoal extends Goal {
	    ...
	    public boolean canUse() {
	        LivingEntity livingentity = FlareEntity.this.getTarget();
	        return livingentity != null ? FlareEntity.this.canAttack(FlareEntity.this.getTarget(), EntityPredicate.DEFAULT) : false;
	    }
	...
	class SweepAttackGoal extends FlareEntity.MoveGoal {
	    ...
	    public void stop() {
	    	if(FlareEntity.this.getTarget() instanceof PlayerEntity) {
	    		FlareEntity.this.setTarget((LivingEntity) null);
	    	}
	        FlareEntity.this.attackPhase = FlareEntity.AttackPhase.CIRCLE;
	    }
	    ...
	    public void tick() {
	        ...
	        if (FlareEntity.this.getBoundingBox().inflate((double) 0.2F).intersects(livingentity.getBoundingBox())) {
