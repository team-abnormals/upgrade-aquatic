...
public class FeatureAmmonite {
	...
	private static void process(Biome biome) {
		if(biome.getCategory() == Category.BEACH || biome.getCategory() == Category.OCEAN) {
			...
			biome.addFeature(Decoration.UNDERGROUND_ORES, Feature.ORE.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.NATURAL_STONE, UABlocks.EMBEDDED_AMMONITE.get().getDefaultState(), 3)).withPlacement(Placement.COUNT_BIASED_RANGE.configure(placement)));
		}
	}
