...
public class FeatureFloweringRush extends Feature<NoFeatureConfig> {
	...
	private static void process(Biome biome) {
		if(biome.getCategory() == Category.RIVER) {
			biome.getFeatures(GenerationStage.Decoration.VEGETAL_DECORATION).add(Biome.createDecoratedFeature(UAFeatures.FLOWERING_RUSH, IFeatureConfig.NO_FEATURE_CONFIG, Placement.COUNT_HEIGHTMAP_DOUBLE, new FrequencyConfig(15)));
		}
	}
}